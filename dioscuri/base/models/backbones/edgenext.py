...
class EdgeNeXt(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 kernel_sizes=[7, 7, 7, 7], classifier_dropout=0.2, heads=[8, 8, 8, 8], use_pos_embd_xca=[False, False, False, False],
...
class EdgeNeXtBNHS(nn.Module):
    def __init__(self, in_chans=3, num_classes=1000,
                 ...
                 drop_path_rate=0.1, layer_scale_init_value=1e-6, head_init_scale=1., expan_ratio=4,
                 kernel_sizes=[7, 7, 7, 7], classifier_dropout=0.0, heads=[8, 8, 8, 8], use_pos_embd_xca=[False, False, False, False],
                 use_pos_embd_global=False, d2_scales=[2, 3, 4, 5]):
        super().__init__()
        for g in global_block_type:
        ...
        # self.head = nn.Linear(dims[-1], num_classes)
        self.head = GDC(dims[-1], num_classes)
...
def edgenext_small(pretrained=False, num_classes=512):
    ...
    model = EdgeNeXt(depths=[3, 3, 9, 3], dims=[48, 96, 160, 304], 
										 num_classes=num_classes, expan_ratio=4,
...
def edgenext_small_bn_hs(pretrained=False, num_classes=512):
    # 5.58M & 1257.28M @ 256 resolution
    # 78.39% Top-1 accuracy
    # For A100: FPS @ BS=1: 174.68 & @ BS=256: 3808.19
    model = EdgeNeXtBNHS(depths=[3, 3, 9, 3], dims=[48, 96, 160, 304], 
												 num_classes=num_classes, expan_ratio=4,
                         global_block=[0, 1, 1, 1],
